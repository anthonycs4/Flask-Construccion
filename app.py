...
def generar_cotizaciones(ubicacion_usuario, presupuesto_max, dias_viaje, cantidad_personas, fecha_inicio, fecha_fin, motivo_viaje):
    """
    Genera cotizaciones de viaje recomendando un hotel, un restaurante y un lugar turístico
    basado en los parámetros del usuario y su presupuesto.
    ...
    :return: Una lista de combinaciones válidas o un diccionario con error si no hay opciones.
    ...
    # Filtrar datos que entren en el presupuesto
    ...
    # Probar con diferentes porcentajes del presupuesto para mayor flexibilidad
        ...
        for _, hotel in hoteles_ajustados.iterrows():
            for _, restaurante in restaurantes_ajustados.iterrows():
                for _, lugar in lugares_ajustados.iterrows():
                    ...
                    # Verificar si entra en el presupuesto ajustado
                    if presupuesto_restante >= 0:
                        # Guardar estadísticas de elección
                        ...
                        break  # Romper al encontrar una combinación válida
                if resultados:
                    break
            if resultados:
                break
...
def api_generar_cotizaciones():
    """
    Endpoint API para generar cotizaciones basadas en los datos proporcionados por el cliente.
    ...
    try:
        ...
        # Llamar a la función generar_cotizaciones, pasando los datos recibidos
        resultados = generar_cotizaciones(**datos)
        
        # Retornar los resultados en formato JSON
        return jsonify(resultados)
    except Exception as e:
        # En caso de un error, imprimir el mensaje de error para depuración
        print(f"Error al procesar los datos: {e}")
        
        # Retornar un mensaje de error en formato JSON y el código de estado 400
